...
Citizen.CreateThread(function()
	while krCB == nil do TriggerEvent('krCB:getSharedObject', function(obj) krCB = obj end) Wait(0) end
    krCB.TriggerServerCallback('coke:processcoords', function(servercoords)
...
Citizen.CreateThread(function()
	while krCB == nil do TriggerEvent('krCB:getSharedObject', function(obj) krCB = obj end) Wait(0) end
    krCB.TriggerServerCallback('coke:startcoords', function(servercoords)
        coord = servercoords
	end)
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not inUse then
			if dist <= 1 then
				sleep = 5
				DrawText3Ds(coord.x, coord.y, coord.z, 'Press [E] to pay $5000 for an untraceable plane')			
				if IsControlJustPressed(1, 51) then
					krCB.TriggerServerCallback('coke:pay', function(success)
						if success then
							main()
						end
					end)
		...
		elseif dist <= 3 and inUse then
			sleep = 5
			DrawText3Ds(coord.x, coord.y, coord.z, 'Somebody else has already requested a plane')
		else
			sleep = 3000
		end
		Citizen.Wait(sleep)
...
AddEventHandler('coke:onUse', function(source)
	if Config.useMythic then
		exports['mythic_notify']:SendAlert('inform', 'Your heart begins to race and you are filled with energy')
	end
	...
	DoScreenFadeIn(2000)
	if Config.useMythic then
		exports['mythic_notify']:SendAlert('inform', 'The effects of the coke seem to have worn off')
	end
...
function main()
	...
	if Config.useMythic then
		exports['mythic_notify']:SendAlert('inform', 'Go to the airfield')
	end
...
function planeGround()
	...
	if Config.useMythic then
        exports['mythic_notify']:SendAlert('inform', 'The plane is out of fuel. Search the area for a jerry can')
    end
...
function planeFly()
	...
	if Config.useMythic then
		exports['mythic_notify']:SendAlert('inform', 'Wait for the plane to land')
	end
	...
	Citizen.CreateThread(function()
		while fueling do
			...
			if disttocoord <= 3 then
				DrawText3Ds(fuelCoords.x,fuelCoords.y,fuelCoords.z, 'Press [E] to pick up the jerry can')
				if IsControlJustPressed(1, 51) then
					TaskTurnPedToFaceEntity(player, fuelSpawn, 3000)
					FreezeEntityPosition(player, true)
					if Config.progBar then
						exports['progressBars']:startUI(1000, 'Picking up jerry can...')
					end
					DoScreenFadeOut(1000)
					Citizen.Wait(1000)
					...
					FreezeEntityPosition(player, false)
					if Config.useMythic then
						exports['mythic_notify']:SendAlert('inform', 'Fill the plane with fuel')
					end
					plane(fuel)
					fueling = false
					dodo = true
	...
	Citizen.CreateThread(function()
		while dodo do
			...
			if disttocoord <= 5 then
				DrawText3Ds(location.parking.x,location.parking.y,location.parking.z, 'Press [E] to fuel up the plane...')
				...
				if IsControlJustPressed(1, 51) then
					...
					if Config.progBar then
						exports['progressBars']:startUI(20000, 'Fueling up the plane...')
					end
					Citizen.Wait(20000)
					if Config.useMythic then
						exports['mythic_notify']:SendAlert('success', 'Finished fueling up the plane')
					end
	...
	while checkPlane do
		...
		if DoesEntityExist(airplane) then
			if GetVehicleEngineHealth(airplane) < 0 then
				if Config.useMythic then
					exports['mythic_notify']:SendAlert('error', 'Canceling')
				end
...
function delivery()
	if Config.useMythic then
		exports['mythic_notify']:SendAlert('inform', 'Get in the plane and pick up the delivery marked on your GPS')
	end
	...
	Citizen.CreateThread(function()
		while delivering do
			...
			if disttocoord <= 20 then
				RemoveBlip(blip)
				SetBlipRoute(blip, false)
				DrawText3Ds(location.delivery.x,location.delivery.y,location.delivery.z-1, 'Press [ E ] to pick up the delivery')
				if IsControlJustPressed(1, 51) then
					delivering = false
					if Config.progBar then
						exports['progressBars']:startUI(Config.pickupTime, 'Picking up the delivery...')
					end
					Citizen.Wait(Config.pickupTime)
					if Config.useMythic then
						exports['mythic_notify']:SendAlert('success', 'Picked up the delivery')
					end
...
function final()
	if Config.useMythic then
		exports['mythic_notify']:SendAlert('inform', 'Deliver the plane back to a hangar')
	end
	...
	Citizen.CreateThread(function()
		while hangar do
			...
			if disttocoord <= 5 then
				RemoveBlip(blip)
				SetBlipRoute(blip, false)
				DrawText3Ds(location.hangar.x,location.hangar.y,location.hangar.z-1, 'Press [ E ] to park the plane')
				...
				if IsControlJustPressed(1, 51) then
					...
					FreezeEntityPosition(airplane, true)
					if Config.progBar then
						exports['progressBars']:startUI(2000, 'Leaving keys...')
					end
					Citizen.Wait(2000)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if dist <= 3 and not isProcessing then
			sleep = 5
			DrawText3Ds(process.x, process.y, process.z, 'Press [ E ] to begin breaking down the cocaine')			
			if IsControlJustPressed(1, 51) then		
				isProcessing = true
				krCB.TriggerServerCallback('coke:process', function(success)
					if success then					
						processing()
					else
						isProcessing = false
...
function processing()
	...
	FreezeEntityPosition(player, true)
	if Config.progBar then
		exports['progressBars']:startUI(6000, 'Breaking down the coke...')
	end
