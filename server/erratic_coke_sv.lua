...
krCB.RegisterServerCallback('coke:processcoords', function(source, cb)
    cb(hiddenprocess)
end)

krCB.RegisterServerCallback('coke:startcoords', function(source, cb)
    cb(hiddenstart)
end)

krCB.RegisterServerCallback('coke:pay', function(source, cb)
...
AddEventHandler("coke:processed", function(x,y,z)
	...
	vRP.tryGetInventoryItem({user_id,"coke",Config.takeBrick,false})
	vRP.giveInventoryItem({user_id,'coke',pick,1})
end)

krCB.RegisterServerCallback('coke:process', function(source, cb)
	local _source = source
	local user_id = vRP.getUserId({_source})
	-- local x = vRP.getInventoryItemAmount(user_id,"coke")
	local check = vRP.getInventoryItemAmount(user_id,"coke")
	if check >= 1 then
    	cb(true)
    else
      if Config.useMythic then
    	 TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = 'You do not have any coke bricks'})
      end
    	cb(false)
	end
end)
...
AddEVentHandler("coke:GiveItem", function()
	local _source = source
	local user_id = vRP.getUserId({_source})
	vRP.giveInventoryItem({user_id,"coke",2})
end)
